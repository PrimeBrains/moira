...
本スキルは本リポ（`PrimeBrains/moira`）側の **authoring ツール**であり、配布物（`moira adapter install` の管理対象）ではない
...
- `moira` CLI 未導入 → 本リポ（`PrimeBrains/moira`）の `moira/cli/README.md` のセットアップを案内して停止。
