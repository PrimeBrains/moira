...
description: >-
  ...
  独立 checker で「Decision↔実装」整合（conformance）を検証する設計。本機構は**定義済。意味突合方式へ改訂済み（issue #1・D-78・2026-07-20）。網羅実走は後続**。
...
# decision-conformance — 設計判断↔実装の整合を独立 checker で検証する（定義済・意味突合方式へ改訂済み・網羅実走/採点脚は後続）
...
**現状は定義済。裏面 ref 方式で 2026-06-27 に D-34／D-40 を試走（ともに ALIGNED）、意味突合方式への改訂は 2026-07-20（issue #1・D-78）。網羅実走は後続ステージ。** 採点ゲートの主体は**方向(A)確定・実装は後続**（詳細は `SKILL.ja.md` 手順5）。

**この英語ファイルは規約シェル。完全な規範手順は [`SKILL.ja.md`](./SKILL.ja.md)**（プロジェクト言語）にある。二重正典を避けるため手順はここに複製しない——`SKILL.ja.md` に従う。
検証アーキテクチャ上の位置づけ（計器⑥は `moira-verification.md` の節「Decisions の検証割付」内）と被覆規律は `.kiro/steering/moira-verification.md`、計器割付の現物は `moira/DECISIONS-CATALOG.md` の各エントリの計器タグが正典（集計表は持たない・D-78）。

一行で: 対象判断を各エントリの計器タグの⑥（＋テスト未整備の①②③④）から選ぶ → 照合対象（実装サーフェス）をユーザーに確定 → `decision-conformance-checker` を並列・独立に派遣（ALIGNED/DRIFTED/UNVERIFIABLE） → DRIFTED を集約し是正先（実装修正 or 目録訂正）にルーティング → 照合結果を報告に残す（目録への状態書き戻しはしない・D-78）。
