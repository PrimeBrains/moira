...
<!-- 規範: .kiro/steering/moira-change-analysis.md §7（一次採取は本 2 欄のみ）・§2（障害判定の基準）。
     ここに記録した 2 欄は、後日の要因分析で出所ラベル `captured`（＝実行時の一次記録）として扱われる。
     **採取するのはこの 2 欄だけ**——検知工程・根本要因まで実行時に書かせない（クローズを重くしない）。
     issue クローズコメントにも同じ 2 欄を短く残す。 -->

- **障害判定**: {{障害|非障害}} — 根拠: {{一言}}
  <!-- 障害＝意図した仕様・約束されたふるまいから外れた状態が実在した（潜在＝未発現も障害）。
       非障害＝要件追加・要件変更・リファクタ・テスト追加・運用変更・仕組み改善・文書のみ。
       境界事例は障害側に倒し、理由を書く。 -->
