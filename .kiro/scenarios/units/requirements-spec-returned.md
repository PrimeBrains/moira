...
<small>※ 「指摘されたぶんだけ出来高を減らす（例 3→2）」という、より細かい見せ方も考えられるが、それには要件定義を**計画時に複数の葉へ分解**しておく必要があり（差し戻しは指摘された葉だけを後退させる）、本ユニットの前提（要件定義は単一ノード）では成立しない。完了後に後付けで葉を割って出来高を按分する案は**敵対ゲートで却下**された——正典上、完了葉への後付け decompose 自体は構造編集として遮断されないが、当該葉は葉基底から外れ EV 寄与が補償まで**消える**（§2.8「I4 が施錠するのは凍結属性であって葉集合の所属ではない」——ゆえに即 3→2 の按分にはならず、まず 3→0 に落ちる）。そこから 3→2 を作るには新しい子葉への見積・合意・完了付与で**確定済みの粒度を遡って作り直す**ことになり、部分EV（v16 却下）の密輸・差し戻しの後退信号（P5）の消去に当たる。完了ノード自身の凍結見積の引き直しは**意味的変更**として完了施錠（I4）が拒む（v21 精密化——施錠が拒むのは意味的変更——の後も同じ: 按分は理解の変更であり、記録の誤りの訂正〔§2.10〕では救済されない。再見積の機構 R-E3 も未完了ノード限定・完了の正道は supersede §2.7）。よって単一ノードの本ユニットでは差し戻しは 3→0（粒度＝計画のコミット判断・MODEL §2.1#4。細粒度版は別ユニットの題材）。詳細は §7。</small>
...
- **完了後の後付け分割・按分は採らない（敵対ゲートで却下）:** 「差し戻し時に完了ノードを kept＋rework に後付け decompose して EV を按分する（3→2）」案は、moira-model-update の独立敵対ゲート（moira-adversary×3・V1–V6）が **I4 完了施錠・R-E3・§2.7 supersede・§2.5 lifecycle に抵触し、部分EV（v16 却下）を密輸し、かつ後退遷移を消して P5 の差し戻し信号を自滅させる**として却下（当時の裁定記録）。条項帰属の精密化（v21 同期）: 完了葉への後付け decompose 自体は正典が構造編集として遮断しない——当該葉が葉基底から外れ EV 寄与が補償まで消える（§2.8——I4 が施錠するのは凍結属性であって葉集合の所属ではない）ため即 3→2 にはならず、3→2 の実現は新子葉への見積・合意・完了付与による**確定済み粒度の遡及的な作り直し**＝部分EV（v16 却下）の密輸・P5 後退信号の消去に帰着する。I4 が拒むのは完了ノード自身の凍結見積の**意味的な引き直し**（R-E3 は再見積の機構で未完了ノード限定・完了の正道は supersede §2.7）。この却下は I4 の v21 精密化（「完了済みは、黙っては変わらない」——記録の誤りの訂正は §2.10 で完了済みにも届く）の後も**不変**: 後付け分割・按分は理解の変更であり、記録の誤りの訂正ではないため、訂正層でも救済されない（issue #7 同期）。**MODEL 変更は不要**（差し戻しは既存の P5 後退で表現し、粒度は計画時の分解で決める）。出所＝本作業の moira-model-update 敵対ゲート・会話ログ。
