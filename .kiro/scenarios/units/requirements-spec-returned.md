...
<small>※ 「指摘されたぶんだけ出来高を減らす（例 3→2）」という、より細かい見せ方も考えられるが、それには要件定義を**計画時に複数の葉へ分解**しておく必要があり（差し戻しは指摘された葉だけを後退させる）、本ユニットの前提（要件定義は単一ノード）では成立しない。完了後に後付けで葉を割って出来高を按分する案は、完了施錠（I4）・完了ノード再見積禁止（R-E3）・supersede 規律（§2.7）に抵触し、かつ差し戻しの後退信号（P5）を消すため**敵対ゲートで却下**された（v21 の普遍訂正原則〔§2.10〕でも却下は不変——後付け分割は計画の**意味的変更**であって「記録の誤り」の訂正ではないため、精密化後の I4「完了済みは、黙っては変わらない」でも施錠が拒む対象）。よって単一ノードの本ユニットでは差し戻しは 3→0（粒度＝計画のコミット判断・MODEL §2.1#4。細粒度版は別ユニットの題材）。詳細は §7。</small>
...
- **完了後の後付け分割・按分は採らない（敵対ゲートで却下・v21 でも不変）:** 「差し戻し時に完了ノードを kept＋rework に後付け decompose して EV を按分する（3→2）」案は、moira-model-update の独立敵対ゲート（moira-adversary×3・V1–V6）が **I4 完了施錠・R-E3（完了ノード再見積禁止）・§2.7 supersede・§2.5 lifecycle に抵触し、部分EV（v16 却下）を密輸し、かつ後退遷移を消して P5 の差し戻し信号を自滅させる**として却下。**v21 の普遍訂正原則（§2.10）でも結論は不変**——後付け分割は「考えの変更」＝**意味的変更**であって「記録の誤り」の訂正ではないため、精密化後の I4（「完了済みは、黙っては変わらない」）でも施錠が拒む対象である（§2.10 の訂正が届くのは記録誤りの修理に限られ、reason 必須＋計器常設表示で音が鳴る；意味的変更の正道は supersede §2.7 のまま）。**MODEL 変更は不要**（差し戻しは既存の P5 後退で表現し、粒度は計画時の分解で決める）。出所＝本作業の moira-model-update 敵対ゲート・会話ログ＋ issue #7（v21 追随）。
