...
**非障害**と判定された変更 1 件ごとの要因分析（17 項目）を置く。**障害**と判定された件は
...
| パス | 内容 |
|---|---|
| `INDEX.md` | 分析済みキーの索引（**障害・非障害の両方**を 1 行ずつ載せる——横断集約の入口を 1 か所にするため） |
| `entries/<repo>-<番号>.md` | 非障害の分析票 本体（1 件 1 ファイル） |
| `reviews/<YYYY-MM-DD>.md` | 横断集約（4 軸頻度・クラスタ・Try 候補と裁定）の結果 |
...
2. **`unknown` は欠損ではなく記録である。** 17 項目のうち 7 項目は既存の入力にそもそも存在しないため
   （issue #19 の実地監査）、埋まらない欄が出るのは正常。**空欄を埋めるための推測を禁じている**結果である。
