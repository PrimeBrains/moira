...
| 工程 | in | 処理 | out | 主担当 |
|---|---|---|---|---|
| **P1 受付・正規化** | 入口 3 種（issue／検出結果／会話依頼） | issue に正規化し、**人間の言葉（曖昧さ・省略を含む）を後段の LLM 処理が迷わない明確な変更要求文に書き起こす**（原文は issue に残る——情報は失わない）。候補クラス（§3）を仮判定。**triage**（裁定済み——来歴は plan §8-3）: フル工程を回すか、軽量ゆえフローを起動せず通常作業へ送り出すかを判定し、判定理由を issue に一言記録。**受付時点の commit を base として変更要求票に記録**（以後の全差分検査は `base..HEAD`）。**full-flow なら issue 専用の git worktree を fresh base（`origin/main`）から作成し、その分岐点を base とする**（軽量 exit は worktree 不要。§7 作業の隔離） | 変更要求票（issue 番号・明確化した要求文・候補クラス・triage 判定・**base commit**／full-flow は worktree 分岐点・ブランチ名） | AI（メイン文脈） |
...
| **P5 同期閉包確認** | 影響マップ＋ゲート証跡＋実差分。**差分の両端を固定**: base＝P1 で記録した変更開始前 commit（full-flow の worktree 運用では worktree 分岐点＝§7）、対象＝P5 開始時の HEAD。**照合中に HEAD が動いたら判定無効・再実行** | 影響マップの各行を resolved／deferred／未了に判定（§5）。`diff(base..HEAD)` に対する未マップ差分の非存在も確認。**deferred 行の後続 issue openness は `gh issue view` で機械照合し証跡化**（人間の確認義務にしない） | **閉包レポート**（人間向け先頭に「3面最終文↔意図の対応表」と「できないことになったこと」の平易な差分を置き、機械決着は折り畳む） | AI（照合 subagent） |
| **P6 クローズ** | 閉包レポート | 人間承認（**H5・薄い**——読むのは §6 の 3 点のみ）→ issue クローズコメント。**あわせて要因分析の一次採取 2 欄**（**障害判定**〔障害／非障害＋一言の根拠〕・**変更分類**）を閉包レポートと issue クローズコメントに記録し、当該 issue を要因分析の未分析キューへ載せる（分析自体はここで走らせない——[moira-change-analysis.md](moira-change-analysis.md) §6/§7）。ジャーナル来歴は**該当する確定分岐がある場合のみ**（記録条件は `moira-model-update`／目録の規律に従う——本フローは条件を新設しない） | クローズ済み issue＋**一次採取 2 欄** | AI＋人間（H5） |

**P2 が使うトレース機構（既存・新造なし）**: MODEL の安定 clause ID（A/I/P/R——行番号禁止）／DECISIONS-CATALOG
の判断文と計器タグ（**変更 path → 関係判断の同定は AI の意味突合。テストコード内 D-N 名指しコメント等を
足場にしてよい**。§5）／PROPERTIES の clause→property 被覆／
...
（full-flow の worktree 運用では **worktree の分岐点**＝fresh `origin/main`。§7 作業の隔離。非 worktree・軽量時は
従来どおり受付時 commit）、HEAD＝P5 開始時に固定）を閉包の必要条件とする。
...
| Decision ↔ 実装の整合（計器⑥） | `decision-conformance`。**対象選定は map→Decision に加え、変更 path からの AI 意味突合（判断文→実装の意味検索・D-78）** | verdict→3値の写像は上記。**照合対象の特定は AI の意味突合**——判断文（決めたこと一文・判定文）を起点に、変更 path が触れる実装サーフェスを意味検索して対応候補を同定し、現物照合する。テストコード内の D-N 名指しコメント・ファイル名・型名を足場にしてよい。**裏面 ref は廃止済み**（issue #1・2026-07-20・D-78）——手維持表の陳腐化に起因する取りこぼし面は消えるが、**意味突合固有の見落としに置き換わる**（「P5 の限界」に計上） |
...
同じ正直さで扱う）: ①M 級変更時の境界モデル検査（第2器・未実装）、②**意味突合の網羅性**（判断文からの対応導出は
探索であり、関係実装の見落としは残る——負の照合の探索範囲明示と陰性対照で緩和。特に**変更 path→判断の
逆方向**は、実装側の D-N 名指しコメント等の足場が薄い判断〔テスト未整備の proposed 等〕で到達性が下がる——
P2 は順方向〔対象クラスの判断文の走査〕と併用して補う）、
...
- **作業の隔離（worktree・full-flow 原則）**: full-flow と判定した issue は、**原則**として fresh な
  `origin/main` から切った issue 専用ブランチの **git worktree**（`.claude/worktrees/issue-N`）の中で
  フロー（P2〜P5）を回す。目的は、複数セッション/タスクが同一チェックアウトの作業ツリー・index を
  共有することに起因する混線（他 issue の staged 作業の紛れ込み・照合中の HEAD/base 移動）の隔離。
  - **機構（`git worktree`）**: まず **`git fetch origin`**（「fresh」は fetch を挟んで初めて成り立つ）→
    `git worktree add .claude/worktrees/issue-N -b worktree-issue-N origin/main`（ブランチ名は
    `worktree-issue-N` に固定）。本 skill は Bash を持つ——ハーネス固有の worktree 機構
    （例: fresh 分岐＋自動 cleanup を持つもの）があればそれを使ってもよい。**worktree 群は
    `.gitignore` の `.claude/worktrees/`** で親チェックアウトから無視する（untracked 汚染・
    `git add -A` での nested checkout 巻き込み・`.claude/worktrees/**` が差分に入って閉包数学を壊す事故の
    予防。§5 の `moira/changes/**` 自己除外と同趣旨）。
  - **base**: worktree の分岐点に固定され、§5 の `diff(base..HEAD)` が当該フローの作業そのものになる
    （base 再アンカーの体操が不要）。full-flow では P1 で記録するローカル HEAD ではなく**この分岐点が正**
    （ローカル HEAD の記録は破棄・分岐点で上書き）。台帳もこの枝上にコミットされる（置き場
    `moira/changes/issue-N/`・非正典性は不変）。
  - **P6 の反映（H5 承認後）**: worktree ブランチを **main へ直マージ**する。origin/main が分岐後に進んで
    fast-forward 不能なら **rebase** し、**rebase 後に P5 の未マップ差分検査を新 base で再走**してから
    push する（並行マージされた作業との**意味的**衝突は本フローの閉包では保証しない——検知器
    〔DRIFTED・CI・E2E〕が拾って新 issue 再入する。§0）。→ push → issue クローズ →**worktree 撤去**
    （`git worktree remove`；未コミット差分が残ると失敗するため、残す作業は先にコミットするか
    `--force` で撤去。merge/push 後・remove 前に落ちて worktree が残った場合は、同 issue 再入時に
    既存 worktree を再利用するか撤去してから作り直す〔`git worktree add` はパス衝突で失敗するため冪等に扱う〕）。
    PR は作らない（レビューは既存ゲート＋H5 が担う）。
  - **適用範囲と非 worktree フォールバック**: worktree は full-flow の**原則**であって強制ではない
    （本フロー自体が紳士協定＝§0）。**worktree を用いない full-flow**（worktree 作成不能な環境・手動運用・
    本規約導入前から in-flight の issue 等）では、従来どおり**受付時 commit を base** とする（§5・§2 P1。
    後方互換）。**軽量 exit の issue は worktree 不要**（P1 で通常作業へ送り出し `base..HEAD` を計算しない）。
  - **正直な限界**: worktree が防ぐのは「別セッション/タスクが**同一チェックアウトを共有して** index に
    他人の作業が紛れる」型の事故（1 worktree = 1 書き手が前提。同一 worktree 内で複数の書き手/subagent が
    stage/commit すれば混線は残る）。main への**並行 push レース**の排他ではない（分岐後 push 競合は上記
    rebase で解く）。
...
方向）に整合（**issue #1 で実施済み・2026-07-20**） ⑤ 検証器パスをリポジトリルート相対のフルパスに統一。

**要因分析フローとの接続（2026-07-25・issue #19）**: 本フローの**出口**（P6 クローズ）が
[moira-change-analysis.md](moira-change-analysis.md)（変更を振り返る工程列）の**入口**になる。本フローが負う義務は
2 つだけ——① P6 で**一次採取 2 欄**（障害判定・変更分類）を記録する（§2 P6 行）② 当該 issue を未分析キューへ載せる
（キューは保存せず算出されるため、実務上は「クローズする」ことが載せることに等しい）。
**要因分析はゲートではない**——閉包判定にも H5 にも関与せず、クローズを遅らせない。なお、各ゲートの
ラウンド内で決着した指摘は要因分析の**入口にしない**（すり抜けギャップ基準・D-84）——
ただし**当該変更自体はクローズ時に母集団へ入る**（入口フィルタは母集団を間引く規則ではない）。

関連: [moira-verification.md](moira-verification.md) ／ [moira-change-analysis.md](moira-change-analysis.md) ／
`moira/DECISIONS-CATALOG.md` ／ `moira/PROPERTIES.md` ／
