...
<!-- 来歴注記（2026-07-21・issue #5）: 本 ADR 起草時点（2026-06-30）は Moira 一式が `sdd-workshop`
リポジトリ内に置かれる前提だったが、2026-07-20 の filter-repo 移管（`PrimeBrains/sdd-workshop` →
`PrimeBrains/moira`。issue #42 相当・移管記録は `moira/changes/issue-42/migration-record.md`）
以後は「CLI 本体は本リポ（`PrimeBrains/moira`）チェックアウトに残る」と読み替える。ADR 本文は
来歴保持のため書き換えない。 -->
