# Moira プロパティ目録（不変条件カタログ）v0.7

状態: **55プロパティ＝52 agreed＋3 proposed（要批准）**。要批准 3 件＝ PR-REPARENT-HEAL・PM-TREE-INV（v20 所属の latest-wins 由来・bb81ff2 で収載済み）・PR-THRASH（R-S3 の畳んだレビュー carve-out 追随のため `agreed` から再降格）。PR-DONE-LOCK・PR-EVENTS-ONLY・PR-CORRECTION-METER は **v0.7 で agreed 昇格**（issue #11・HA 一文批准。PR-CORRECTION-METER の★は HB 裁定 F2 に従い③a 判定・③c 検知の実装到達をもって解除済み〔issue #11 実装工程・2026-07-22〕）。v0.3 時点の52件は全件人間批准済み。方針は [.kiro/steering/moira-verification.md](../.kiro/steering/moira-verification.md)、正典は [MODEL.md](MODEL.md)。
...
| **PR-DONE-LOCK** | I4・R-E3・§2.10・§7#19(c) | いったん**完了して出来高が確定した**作業の**凍結された額**（そのノード自身の EV_abs）は、**見積のやり直し・作り直し・キャンセルなど「考えが変わった」操作では減らない**（キャンセルで active→sunk に分類が移ることはあるが額は不変——分類は PR-CANCEL-SUNK。完了済みを変えたいときは**新しい行を立てて差し替え**）。唯一の例外は**記録の誤りの訂正**（理由必須・計器に常設表示される「音の鳴る」修理）——**そのノード自身の凍結額が**黙って減る経路は存在しない。ただし**集計への寄与**は所属の再編（完了済みの葉に子を与える等）で動きうる——正典が開示する既知の残余であり、本一文の守る対象ではない。見積のやり直しで出来高や率が動くのは、**まだ完了していない**作業だけ | そのノード自身の凍結 EV_abs 額は訂正以外で不変・訂正経由の変化は計器に常設表示 | 未完了ノードの EV%・カバレッジ、および完了ノードの**集計への寄与**（所属再編で動きうる——§7#19(c) の開示済み残余） | agreed |
...
| **PR-EVENTS-ONLY** | R-U2・A2・R-U1・§2.8・§2.10 | 状態の変化は **4 種類の追記イベント＋記録の訂正（追記専用の第二層）だけ**で起こり、保存済みの状態やイベントを直接書き換えることはできない（同じイベント列＋同じ訂正列を再生すれば同じ状態になる。訂正はイベントを書き換えず「読み」を補正する） | 状態変化=4 イベント＋訂正層の適用後読みのみ・直接変更拒否・再生決定的 | イベント内容 | agreed |
...
| **PR-CORRECTION-METER** | §2.10 (d)・§3・§2.1 会計規律 | 訂正の**数え方に裁量ノブを持たせない**——4 区分（訂正総数・施錠対象への訂正・遡及・適用不能訂正）は常に見える形で数え、UI で畳む・沈めるは可でも、特定の訂正を「技術的訂正」等の名目で**数から除いてよいことにはしない**。閾値・色分けの裁量ノブなし。issue #36 の遡及書き込み警告は本計器の③遡及に統合され、二重表示を避ける（訂正跨ぎと遡及書き込みは同じ計器を鳴らす。遡及書き込み側は観測層の検知——**検知できたものは除外できない**が、検知の被覆限界は正典の開示に従う） | 4 区分の計数から要素を除外するオプションを実装が持たない（③の追記系は検知できたものについて）・閾値/色分けの裁量ノブなし・「畳む/沈める」は UI 層のみで計数不変・遡及書き込み警告は③遡及に統合 | UI での畳み方・並び順・色 | agreed |
...
| R-S1 | 被覆済み | 完了時に見積寄与を凍結＝I4。**PR-DONE-LOCK**（そのノード自身の凍結額の不変〔集計寄与は所属再編で動きうる——§7#19(c)〕）が bind |
...
| A2 | PR-CANCEL-SUNK・PR-EVENTS-ONLY・PR-REPARENT-HEAL | 追記専用——PR-EVENTS-ONLY が直接 bind、PR-CANCEL-SUNK がコスト保持・PR-REPARENT-HEAL が「補償も追記」で間接。v21: A2 射程改訂（導出=ログ＋訂正層 §2.10）は PR-EVENTS-ONLY の改訂一文が追随（**v0.7 で agreed**） |
...
| I4 | PR-DONE-LOCK・MC-UNAGREED-DONE | 完了施錠・未合意完了の空虚成立。v21: 施錠の精密化（「黙っては変わらない」——訂正 carve-out）は PR-DONE-LOCK の改訂一文が追随（**v0.7 で agreed**）。**§2.10 訂正層自体の bound プロパティ**は v0.6 で **PR-CORRECTION-METER ★**（計器 4 区分・裁量ノブなし）を起票し **v0.7 で agreed**（★は issue #11 実装工程の③a 判定・③c 検知到達で解除済み・2026-07-22）。訂正の意味論一行原理（適用後ログは既存意味論でそのまま読み直せる）は PR-EVENTS-ONLY の改訂一文が bind（**v0.7 で agreed**） |
...
| clause | bound by | 備考 |
|---|---|---|
| R-S1 | （D 節：被覆済み=I4） | 完了時凍結（そのノード自身の凍結額——集計寄与の残余は §7#19(c)）。PR-DONE-LOCK が bind |
...
## v0.6→v0.7 変更点（2026-07-21・issue #11 bound プロパティ再批准＋MODEL v22 追随）

MODEL v21→v22（§2.10 (d) ③遡及の定義一本化——一つの警告面の下の二系: 訂正系③a/③b〔会計層〕・追記系③c〔観測層の検知〕。旧 issue #36 遡及記録警告の統合・HB 裁定 F1）への bound プロパティ同一 run 再批准（moira-model-update ゲート内・変更管理フロー issue #11 の HA 事前批准 2026-07-21）。プロパティの新規追加・削除はなし。

1. **PR-DONE-LOCK・PR-EVENTS-ONLY・PR-CORRECTION-METER を `agreed` へ昇格**: PR-EVENTS-ONLY は v0.5 起票の文面のまま・PR-CORRECTION-METER は v0.6 起票文面に区分名の一語同期と検知開示句を加えた文面（変更点 3）・PR-DONE-LOCK は HB F3 の改稿文面（変更点 4）——いずれも HA/HB の人間批准。昇格前提（issue #6 の実装完了後に PBT の非空虚性 witness を得る）は充足——done-lock.pbt.test.ts（v21 carve-out 3 件）・correction.test.ts（§2.10 (a)〜(h)・(g) 計器 4 件・意味論一行原理）、いずれも CI 実走 green。
2. **PR-CORRECTION-METER の★は HB 裁定 F2（2026-07-21）どおり実装到達で解除**: 批准時点の実装到達は①②③b④まで（③a 判定・③c 検知は未実装）のため★を維持し、issue #11 実装工程で③a 判定（fold の per-record 評価・①全数計上の是正込み）と③c 検知（CLI report・観測層・24h 超境界）が到達した 2026-07-22 に解除した。agreed 昇格（一文の人間批准）と★（実装到達の印）は直交——★付き agreed の前例は PR-BUFFER-CLAMP ★。
3. **v22 ③二系化の一文への同期（ゲート R1 採点 FAIL・R2 指摘の是正）**: 一文の 4 区分列挙を「遡及訂正」→「**遡及**」へ一語同期（MODEL §2.10 (d)・NAMING の v22 改称に追随）し、統合句に**検知開示句**（「遡及書き込み側は観測層の検知——検知できたものは除外できない・被覆限界は正典の開示に従う」）を追加（R2 指摘 B3/C-R2-3: 一語同期が無除外主張を観測層③c へ拡幅したため、係り先を一文内で明示）。この同期は HB 裁定 F1（「③は一つの『遡及』警告面の下に訂正系③a/③b と追記系③c を系ごとに計数して共表示」）が実質的に決めている派生同期であり、新たな意味判断を含まない。一文内の「issue #36 の遡及書き込み警告は本計器の③遡及に統合され（訂正跨ぎと遡及書き込みは同じ計器を鳴らす）」句は F1 裁定そのもの。一文の主語「訂正の数え方」の会計保証は③の訂正系＋検知できた③c に係る（MODEL 会計規律と同じ係り先）。実行 witness: ③a/③b・1 記録 1 カウント・①全数計上は correction.test.ts（v22 拡張・23 件）、③c 検知と単一警告面は CLI report.test.ts/correct.test.ts が覆う（issue #11 実装工程で到達・2026-07-22）。
4. **PR-DONE-LOCK の一文を締めて昇格**（HB 裁定 F3・2026-07-21）: 敵対ゲート R1（codex #9→著者反証→再反論で反証 UNSOUND）で、旧一文の「黙って減る経路は存在しない」が §7#19(c)（完了合意済み葉の内部化で集計寄与が補償まで消える——訂正計器は鳴らない）に反証されると確定。守る対象を「そのノード自身の凍結額」へ限定し、集計寄与の所属再編残余を一文内で正直開示する改稿文面を HB で批准して agreed 化。
5. **被覆表の同期**: A2 行・I4 行の（proposed）注記を v0.7 agreed へ更新。

版の規律: 批准状態に触れるため v0.7 に上げる（件数は 55 のまま）。
...
1. **v0.7 の要批准 3 件**: PR-REPARENT-HEAL・PM-TREE-INV（v20 所属の latest-wins 由来）・PR-THRASH（R-S3 の畳んだレビュー carve-out 追随・`agreed` からの再降格）。一文レビューで `agreed` へ。（PR-DONE-LOCK・PR-EVENTS-ONLY・PR-CORRECTION-METER は v0.7 で昇格済み——issue #11 の HA 一文批准で決着。PR-CORRECTION-METER の実行 witness 被覆は現状③b 側のみ〔③a 判定・③c 検知は issue #11 実装工程で witness 追加・★解除〕。）
