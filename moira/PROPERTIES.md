...
状態: **55プロパティ＝52 agreed＋3 proposed（要批准）**。要批准 3 件＝ PR-REPARENT-HEAL・PM-TREE-INV（v20 所属の latest-wins 由来・bb81ff2 で収載済み）・PR-THRASH（R-S3 の畳んだレビュー carve-out 追随のため `agreed` から再降格）。PR-DONE-LOCK・PR-EVENTS-ONLY・PR-CORRECTION-METER は **v0.7 で agreed 昇格**（issue #11・HA 一文批准。PR-CORRECTION-METER の★は③a 判定・③c 検知の実装到達〔issue #11 実装工程〕で解除——HB 裁定 F2）。v0.3 時点の52件は全件人間批准済み。方針は [.kiro/steering/moira-verification.md](../.kiro/steering/moira-verification.md)、正典は [MODEL.md](MODEL.md)。
...
| I4 | PR-DONE-LOCK・MC-UNAGREED-DONE | 完了施錠・未合意完了の空虚成立。v21: 施錠の精密化（「黙っては変わらない」——訂正 carve-out）は PR-DONE-LOCK の改訂一文が追随（**v0.7 で agreed**）。**§2.10 訂正層自体の bound プロパティ**は v0.6 で **PR-CORRECTION-METER ★**（計器 4 区分・裁量ノブなし）を起票し **v0.7 で agreed**（★は③a 判定・③c 検知の実装到達〔issue #11 実装工程〕で解除）。訂正の意味論一行原理（適用後ログは既存意味論でそのまま読み直せる）は PR-EVENTS-ONLY の改訂一文が bind（**v0.7 で agreed**） |
