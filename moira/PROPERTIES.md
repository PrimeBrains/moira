# Moira プロパティ目録（不変条件カタログ）v0.6

状態: **55プロパティ＝49 agreed＋6 proposed（要批准）**。要批准 6 件＝ PR-REPARENT-HEAL・PM-TREE-INV（v20 所属の latest-wins 由来・bb81ff2 で収載済み）・PR-THRASH（R-S3 の畳んだレビュー carve-out 追随のため `agreed` から再降格）・**PR-DONE-LOCK・PR-EVENTS-ONLY（v21 普遍訂正原則〔§2.10 訂正層〕追随のため `agreed` から再降格——改訂＝再批准の自己規律。issue #2）**・**PR-CORRECTION-METER ★（v21 §2.10 (d) の訂正計器 4 区分・裁量ノブなしを一文起票。実装 pending・issue #6）**。v0.3 時点の52件は全件人間批准済み。方針は [.kiro/steering/moira-verification.md](../.kiro/steering/moira-verification.md)、正典は [MODEL.md](MODEL.md)。
...
| **PR-CORRECTION-METER ★** | §2.10 (d)・§3・§2.1 会計規律 | 訂正の**数え方に裁量ノブを持たせない**——4 区分（訂正総数・施錠対象への訂正・遡及訂正・適用不能訂正）は常に見える形で数え、UI で畳む・沈めるは可でも、特定の訂正を「技術的訂正」等の名目で**数から除いてよいことにはしない**。閾値・色分けの裁量ノブなし。issue #36 の遡及書き込み警告は本計器の③遡及に統合され、二重表示を避ける（訂正跨ぎと遡及書き込みは同じ計器を鳴らす） | 4 区分の計数から要素を除外するオプションを実装が持たない・閾値/色分けの裁量ノブなし・「畳む/沈める」は UI 層のみで計数不変・遡及書き込み警告は③遡及に統合 | UI での畳み方・並び順・色 | proposed（v21 §2.10 追随の新規起票・実装 pending・issue #6） |
...
| I4 | PR-DONE-LOCK・MC-UNAGREED-DONE | 完了施錠・未合意完了の空虚成立。v21: 施錠の精密化（「黙っては変わらない」——訂正 carve-out）は PR-DONE-LOCK の改訂一文が追随（proposed）。**§2.10 訂正層自体の bound プロパティ**は v0.6 で **PR-CORRECTION-METER ★**（計器 4 区分・裁量ノブなし）を起票（proposed・実装 pending・issue #6）。訂正の意味論一行原理（適用後ログは既存意味論でそのまま読み直せる）は PR-EVENTS-ONLY の改訂一文が bind（proposed） |
...
未 bound（可視ギャップ）は v0.3 で **0** になった——全 clause が「falsifiable プロパティで bound」か「D 節で構造・メタとして明示分類」のいずれかに割り当てられた。**ただし bound のうち ★impl-pending（R-T3 過負荷・R-T6 バッファの数値導出〔残量・消費率〕・R-S7 の標識発火・§2.10 訂正計器 4 区分）は参照実装に当該導出が未実装**であり、実行テスト化は実装到達後（仕様＝MODEL に対しては今すぐレビュー可）。**R-T4 期日アラートは実装到達済み（issue #13・2026-07-04）**——着地曲線の導出（backend）＋期日判定（frontend の純導出モジュール・超過量付き・コミットで消滅）が実在し、★ を外した（MC テスト化は後続）。**R-C3 キャンセル孤児も基本形が実装到達済み（issue #12）**——中止先行の直接後続への警告（frontend 純導出・コミットで消滅）が実在し ★ を外した。ただし辺閾値の葉述語まで含む「永久充足不能」の完全意味論は未実装（正直な限定・MC テスト化は後続）。**D_pred の集約 read は着地導出（issue #13）で、クリティカルパスの read はガント強調（issue #16・公開導出）で実現済み**——v0.3 時点の「未実現は D_pred 集約 read とバッファ可視化の側」のうち残るのは**バッファ残量・消費率の数値導出＋§2.10 訂正計器 4 区分（issue #6）**。**批准の進捗**: v0.3 の52プロパティは全件 `agreed`。v0.4 時点は **54 プロパティ＝51 agreed＋3 proposed**（v20 追加の PR-REPARENT-HEAL・PM-TREE-INV、R-S3 追随で再降格の PR-THRASH——要批准）。v0.5 で **54＝49 agreed＋5 proposed**（v21 追随で PR-DONE-LOCK・PR-EVENTS-ONLY を再降格）。v0.6 で **55＝49 agreed＋6 proposed**（v21 §2.10 (d) 追随で PR-CORRECTION-METER ★ を新規起票）。
...
## v0.5→v0.6 変更点（2026-07-21・issue #6 MODEL v21 §2.10 訂正計器の bound プロパティ起票）

MODEL v21 §2.10 (d)「訂正計器の見える化」を bind する新規プロパティを起票（issue #6・HA 意図批准 R13。実装 pending）。件数を 54→55 に更新（proposed 6 件）。

1. **PR-CORRECTION-METER ★ の新規起票（proposed）**: 「訂正の数え方に裁量ノブを持たせない——4 区分（総数・施錠対象・遡及・適用不能）は常設で、畳む/沈めるは可でも計数から除いてよいことにはしない」。根拠 §2.10 (d)・§3・§2.1 会計規律。issue #36 の遡及書き込み警告は本計器の③遡及に統合する方針（HA B5 裁定）。参照実装は issue #6 完了まで未実装のため ★impl-pending。
2. **被覆表の同期**: I4 行の「§2.10 訂正層自体の bound プロパティは未起票」を「v0.6 で PR-CORRECTION-METER ★ を起票」へ更新。
3. 状態: 54→55 プロパティ・要批准 5→6 件。

版の規律: 件数に触れるため v0.6 に上げる。
...
1. **v0.6 の要批准 6 件**: PR-REPARENT-HEAL・PM-TREE-INV（v20 所属の latest-wins 由来）・PR-THRASH（R-S3 の畳んだレビュー carve-out 追随・`agreed` からの再降格）・**PR-DONE-LOCK・PR-EVENTS-ONLY（v21 訂正層追随・`agreed` からの再降格）**・**PR-CORRECTION-METER ★（v21 §2.10 (d) の新規起票・実装 pending）**。一文レビューで `agreed` へ（実装 pending 3 件〔PR-DONE-LOCK・PR-EVENTS-ONLY・PR-CORRECTION-METER〕は issue #6 の C 群実装完了後に PBT の非空虚性 witness を得てから昇格）。
