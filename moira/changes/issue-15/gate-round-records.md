...
## D 面ゲート（R6・R7 / doc-refine・DECISIONS-CATALOG D-79/D-1 実装状態注記）

### ドラフト（未コミット・append-only 追補 2 行）

- 著者: 主コンテキスト。D-79・D-1 に「参照実装の実装状態（2026-07-23・issue #15 実装工程完了）」
  注記を各 1 行追記。判断一文・判定文・既存注記は不変。一次資料セットは HA ⑤で確定済み
  （SOURCE_SET_CONFIRMED の根拠）。

### ラウンド 1（敵対 Claude 1＋codex 1・fact-check・2026-07-23）

- codex: Critical 0・**Important 1**（「常に受理」が幽霊 nullify を含む過剰主張——実装は kind
  判定前に幽霊を④計上で除外）→ MODEL 追補と同一ヘッジ「入口検証では拒否されず(幽霊訂正の
  可視ギャップ扱いは従前どおり)」へ修正済み。FYI: 実装照合一致・append-only（numstat 2 0）
- doc-fact-checker: **NO_OBJECTION**——全 9 主張 CONFIRMED（テスト実走 78 件 PASS 含む・
  append-only 2 追加 0 削除・issue #16 OPEN・MODEL 312f954 相互整合・R9 裁定整合）
- doc-adversary（G1〜G4）: Critical 0・Important 3・Suggestion 3・FYI 1・FORK 1
  - I-1（append-only を自身で検証不能——ツール制約）→ **交差証跡で決着**: codex の
    git 照合（numstat 2 0・削除行なし）＋fact-checker claim 8 CONFIRMED
  - I-2/FORK（D-80 読み替え規律と mixed-provenance——07-23 注記が本リポ #16/#11 を無注記
    導入）→ **批准記録 R6(2)「新旧 #15 の読み分け注意（D-80）は保持」が「読者が新旧番号を
    読み分けられること」への意図を実質的に含意**——選択肢 B（07-22 の #15 手当てと同型の
    インライン注記。D-80 本文には触れない＝選択肢 A は今回の批准スコープ外）で被覆し、
    本リポ番号の明示注記を追補末尾に追加した。被覆判定の健全性は採点者の fork 被覆監査対象
  - I-3（F1/F2 の HB 状態——人間批准済みか）→ **決着**: F1（2026-07-23）・F2（2026-07-23）
    とも AskUserQuestion による人間の実裁定（本セッション記録）。HB のバッチ規律は
    「下流をブロックする時点で諮る」（steering §6）に従い、F1 は C ゲート完了を、F2 は
    M ゲート採点をブロックする時点で諮った（細切れ批准ではなくブロック時点フラッシュ）
  - S-1（「列挙 4 例」の閉集合誤特徴づけ——§2.10 は「等」つき非網羅）→ 「明示 4 例
    （『等』つき非網羅列挙の例示）」＋「試行 fold は列挙に限らない一般検証」へ修正済み
  - S-2（「④⑤の帰結」の帰属不正確——当該保証は無番号帰結文）→ 帰結文の直接引用へ修正済み
  - S-3（D-1 注記の最小化余地）→ 2 項目の再列挙を圧縮（R7「最小の追補」へ接近）
  - FYI（平易言語規約と実装注記ジャンルの既存緊張）→ 記録のみ（07-23 が新たに破ったものでない）
