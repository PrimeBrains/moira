...
| R7 | moira/cli/src/（訂正の書き込み UX） | C | MODEL §2.10 (a)(b)(c) 訂正記録の書き込み層（追記専用・reason 必須・元 id 名指し）／issue-2 R14 | /kiro-impl＋codex＋CI | CLI から訂正を書き込む UX を実装（型に沿った append・reason 必須の入力検証） | 単体テスト・codex レビュー・CI | **deferred** | 後続 issue #11（OPEN・機械照合済 2026-07-21）・owner nakawodayo・再評価条件: 対話的 CLI コマンド設計に着手した時点で issue #11 で moira-change フローを回す。backend 型（Correction）は R1（commit cbfdeb8）で用意済み |
| R8 | moira/frontend/src/moira/warnings.ts | C | MODEL §2.8 矛盾所属警告（R-U12 同型・decompose actor 別最新比較）・§2.10 (d) 訂正計器 4 区分／issue-2 R15 | /kiro-impl＋codex＋CI | 矛盾所属警告カテゴリを追加。訂正計器 4 区分の警告述語 | 単体テスト・codex レビュー・CI | **resolved** | commit a1242bd で 3 段実装完了: (1) backend types.ts の ProjectedNode に claimedParentByActor: Map<string, NodeId> 追加、(2) fold.ts の applyDecompose で human actor のみレジスタ更新、(3) frontend warnings.ts に R-U12-parent 警告述語追加。warnings.test.ts 新規 4 件 pass（発火・追認 clear・agent 除外・単独 silent） |
| R9 | moira/frontend/src/surfaces/health/HealthSurface.tsx | C | MODEL §2.10 (d) 訂正計器の**常設 4 区分**表示（総数・施錠対象・遡及・適用不能）・§3 I4 施錠対象訂正の常設表示／issue-2 R15 | /kiro-impl＋codex＋CI | 訂正計器 4 区分の常設ゾーンを追加。閾値・色分けの裁量ノブなし（畳む/沈めるは可、計数除外は不可） | 単体テスト・snapshot・codex レビュー・CI | **resolved** | commit a1242bd で zone 3 として訂正計器 4 区分 Card 追加。SummaryStat 4 件（総数・施錠対象・遡及・適用不能）・locked/retroactive/inapplicable の tone 分岐・「計数除外なし」の説明文明示。全 frontend 172 tests pass |
| R10 | moira/cli/src/report.ts | C | MODEL §2.10 訂正跨ぎ Δ の隠さない提示・既存の遡及記録警告（issue #36）との統合／issue-2 R15 | /kiro-impl＋codex＋CI | Δ（前営業日比等）が訂正を跨ぐ場合は「訂正跨ぎ」を明示。訂正計器 4 区分を report に反映。既存の遡及警告を訂正計器に統合または連動させる | 単体テスト・codex レビュー・CI | **resolved** | commit a1242bd で ReportOptions.corrections?・ReportJson.correctionMeter を追加、renderReport が cm.total > 0 の時のみ「⚠ 訂正記録 総数 N 件」行を出力（施錠対象/遡及/適用不能の内訳付き）。CLI 302 tests pass。**注記**: HA B5 統合（issue #36 遡及警告と③retro の完全統合）は現状「配線は今後」で残る（後続 issue #11 で完全配線） |
| R11 | moira/PROPERTIES.md（PR-DONE-LOCK） | P | MODEL I4 v21 精密化・D-1・v21 追随のため PR-DONE-LOCK は proposed へ再降格済み——本実装完了で再批准（訂正経由の変化は計器常設表示の性質を含めた一文） | HA 意図批准（一文）＋PBT 実装は R4 で担う | PR-DONE-LOCK の一文を v21 訂正 carve-out を含めて改訂し agreed へ再昇格 | HA 批准記録＋R4 の PBT green＋bound プロパティ再批准（同一 run） | **deferred** | 後続 issue #11（OPEN・機械照合済 2026-07-21）・owner nakawodayo・再評価条件: R7 実装完了後に bound プロパティ同一 run 再批准で決着。PBT witness は cbfdeb8 で完備（done-lock.pbt.test.ts v21 carve-out 3 件） |
| R12 | moira/PROPERTIES.md（PR-EVENTS-ONLY） | P | MODEL R-U2・A2 射程改訂・§2.8・§2.10・v21 追随のため PR-EVENTS-ONLY は proposed へ再降格済み——本実装完了で再批准（訂正層の適用後読みを含めた一文） | HA 意図批准（一文）＋PBT 実装は既存 R-U2 pbt を訂正対応拡張 | PR-EVENTS-ONLY の一文を「4 イベント＋訂正層の適用後読みのみで状態が決まる」へ改訂し agreed へ再昇格 | HA 批准記録＋PBT green＋bound プロパティ再批准 | **deferred** | 後続 issue #11（OPEN・機械照合済 2026-07-21）・owner nakawodayo・再評価条件: R7 実装完了後の再批准で決着。意味論一行原理 witness は correction.test.ts（cbfdeb8）で完備 |
| R13 | moira/PROPERTIES.md（新規: 訂正計器 4 区分の常設） | P | MODEL §2.10 (d)・§3 I4 の計器規律「閾値・色分けの裁量ノブなし。畳む/沈めるは可、計数除外は不可」を一文の bound property として起票 | HA 意図批准（一文）＋PBT 実装（新規） | 「訂正計器 4 区分は常設で計数から除外できない」property を新規起票・agreed 化 | HA 批准記録＋新規 PBT green | **deferred** | 後続 issue #11（OPEN・機械照合済 2026-07-21）・owner nakawodayo・再評価条件: R7 実装完了後の再批准で決着。一文起票は feeecff で完了（PROPERTIES v0.6・proposed）・PBT witness は done-lock.pbt.test.ts「計数除外オプションなし」テスト（cbfdeb8）で完備 |
...
| R19 | moira/frontend/src/moira/warnings.test.ts | C | R8 実装（矛盾所属警告）の非空虚性 witness・追認 clear・agent 除外・単独 silent の 4 条件 | /kiro-impl＋codex＋CI | R-U12-parent 警告の発火・clear・edge case をテストで固定 | テスト green・CI | **resolved** | 未マップ差分検出時に追記（append-only §5）。commit a1242bd で warnings.test.ts に 4 件（R-U12-parent describe ブロック）追加・frontend 172 tests pass |
| R20 | moira/frontend/src/surfaces/portfolio/person-overlap.test.ts | C | R1（ProjectedState への correctionMeter 追加）と R8（ProjectedNode への claimedParentByActor 追加）に伴う test literal の同期 | /kiro-impl＋codex＋CI | 既存テストが新型必須フィールドを含むよう追記（挙動不変） | tsc pass・テスト green | **resolved** | 未マップ差分検出時に追記。commit db609ba で correctionMeter を、a1242bd で claimedParentByActor を追加。frontend tsc 0 errors・168→172 tests pass |
| R21 | moira/cli/src/commands-write-safety.test.ts・dates.test.ts・milestone.test.ts・adapter/drift/drift-golden.test.ts | C | CLI テスト isolation バグ（親シェルの MOIRA_DIR を継承しユーザーの実 workspace を読む）の恒久修正——本 issue の P4 実装検証中に発見された pre-existing bug の是正 | /kiro-impl＋codex＋CI | 各 beforeEach/beforeAll で MOIRA_DIR を保存・delete し afterEach/afterAll で復元（既存 adapter/hooks.test.ts と同型 pattern） | CLI テスト green（`env -u MOIRA_DIR` 不要化） | **resolved** | 未マップ差分検出時に追記。commit 7ccdfd3 で 4 ファイル修正・CLI 271 pass + 31 fail → **302 pass + 2 skipped**（本 issue のスコープ外だが実装検証で発覚した pre-existing bug の場での修理） |
| R22 | .kiro/scenarios/units/requirements-spec-returned.md・schedule-rebaseline.md | S | issue #7（シナリオ文言同期）の管轄。本 issue の P5 開始時点で main に含まれる差分（commit 8a93f57・issue #7 の P4→P6 閉包 PASS 済み） | 本 issue の管轄外——**issue #7 の impact-map が別途 map** | 本 issue が触らない（触っていない）ことの明示的開示。issue #7 が既にクローズ済みで担保 | issue #7 の closure-report が担保 | **resolved** | 本 issue の commits（feeecff/cbfdeb8/db609ba/7ccdfd3/a1242bd）はこれらのファイルを touch していない（commit 8a93f57 = issue #7 が touch）。base..HEAD の全差分検査で顕在化するが、issue #7 の flow が既に閉包 PASS で担保しており本 issue の未マップとしては扱わない（正直開示） |
