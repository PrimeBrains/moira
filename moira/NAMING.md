...
| **訂正計器** | correction instrument (§2.10, v21) | 訂正の常設 4 区分（総数・施錠対象への訂正・遡及訂正・適用不能訂正）を health の導出会計に常設表示する計器。閾値・色分けの裁量ノブを持たない | ①②③はスタンプ済み記録の単調計数、④のみ現在状態の述語。提示から除外不可（§2.1 会計規律） |
| **遡及訂正** | retroactive correction (§2.10, v21) | 対象イベントの ts を現行の読みより過去へ動かす訂正。訂正計器の専用常設区分 | 裸の backfill（無警告の遡及記録）は書き込み層で拒否のまま——過去を直す正しい道は音の鳴る訂正 |
