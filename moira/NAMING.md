...
| **訂正記録** | correction record (§2.10, v21) | 第一層イベントの記録の誤りを修理する第二層の追記（追記専用・reason 必須・元イベント id 名指し・ts 付き）。値の訂正／誤記の表明の二形。対象イベント単位の latest-wins | 「記録が誤っていた」の修理。**補償**（考えが変わった＝latest-wins 再提案・cancelled・supersede 等）とは別チャネル（§2.10「補償との区別」） |
| **訂正計器** | correction instrument (§2.10, v21→v22) | 常設 4 区分（総数・施錠対象への訂正・遡及・適用不能訂正）を health の導出会計に常設表示する計器。閾値・色分けの裁量ノブを持たない | ①②と③の訂正系はスタンプ済み記録の計数（単一ログの追記履歴で単調・分割ログのマージ時は再評価されうる）、③の追記系は観測層の検知（被覆限界を開示）、④のみ現在状態の述語。除外不可は会計層の計数と検知できた③c に係る（§2.1 会計規律） |
| **遡及**（訂正計器③） | retroactive (correction-meter category ③; §2.10, v21→v22) | 過去を書き換える読みを生みうる記録の常設区分——**一つの警告面の下の二系**で計数（単一合算数にしない）: 訂正系＝③a 対象の ts を過去へ動かす訂正・③b 対象の ts より 1 日超後着の訂正（会計層・単調・除外不可）／追記系＝③c 起きたとされる ts より 1 日超後の遡及イベント追記（観測層の検知・被覆限界を正直開示。旧 issue #36 遡及記録警告を統合——訂正跨ぎと遡及書き込みは同じ警告面で鳴る） | 裸の backfill（実績日付が lifecycle 最終 ts より過去の遡及記録）は書き込み層で拒否のまま——過去を直す正しい道は音の鳴る訂正。「1 日」＝経過 24 時間超の固定の定義境界であり裁量ノブではない |
| **解除センチネル** | clearing sentinel (§2.8, v21) | 被割当者・レビュー担当を未割当・未指名へ戻す明示の解除値（状態保持遷移に載る）。言及の欠落は不変・明示解除のみが解除 | assignee/reviewer は別属性・別レジスタで独立解除。frozenSlot は動かさない（§3・R-S7 検知対象のまま） |
...
> **追記（2026-07-21・moira-model-update 独立敵対ゲート PASS・MODEL v21→v22・issue #11）:** 訂正計器の③区分を「遡及訂正」から「**遡及**」へ改称・定義一本化（一つの警告面の下の二系＝訂正系③a/③b・追記系③c。旧 issue #36 遡及記録警告の統合）。上表 2 行（訂正計器・遡及）を同期（MODEL 本体と同一 run で残存 Critical/Important = 0 のゲートを通過）。
>
> **追記（2026-07-20・moira-model-update 独立敵対ゲート PASS・MODEL v20→v21・issue #2）:** 上表に v21 語彙 4 行（**訂正記録・訂正計器・遡及訂正・解除センチネル**）を追加（MODEL 連動の同期。残存 Critical/Important = 0 のゲートを MODEL 本体と同一 run で通過）。
>
