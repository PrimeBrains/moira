...
正典モデルは [`MODEL.md`](./MODEL.md) を**単一の真実源(SSOT)**とする。現在の版は **v21**
(普遍訂正原則——すべてのイベントの記録は「追記のみ・理由必須・元記録の名指し・計器に常設表示・
対象は記録であって事実ではない」の統制の下で訂正可能——を第二層の訂正記録として正典化した版。
issue #33 分析→issue #2 起点。実装同期は issue #6)で、独立した敵対者による falsifiable な
確定ゲート(独立採点者の残存 Critical/Important = 0)を通って確定している。
...
```bash
# 1度だけ(本リポの git 根 <clone>/moira/ から実行): backend/frontend をビルド → CLI を npm link
cd <clone>/moira        # 前節の pwd（<clone>/moira/moira/frontend 等）から git 根へ戻る
...
> **移管前文書中の `#N` について**: 本リポは 2026-07-20 に旧リポ（`PrimeBrains/sdd-workshop`）から
> `git filter-repo` で移管された（[issue #42](https://github.com/PrimeBrains/moira/issues/42) 相当・
> 移管記録は [`changes/issue-42/migration-record.md`](./changes/issue-42/migration-record.md)）。
> **本リポに残る移管前文書中の `#N` は `PrimeBrains/sdd-workshop#N` を指す**——本文の機械置換は
> 行わない（来歴主義）。読み替え方針の canonical は [`HISTORICAL-REFERENCES.md`](./HISTORICAL-REFERENCES.md)。
