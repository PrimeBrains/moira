...
EVM 導出（`moira show`）と React ダッシュボード（`moira ui`）を立ち上げる。CLI 本体は本リポ
（`PrimeBrains/moira`）チェックアウトに残り、各リポジトリは `.moira/`（イベントログ等）だけを持つ
（git/eslint と同じモデル）。
...
```bash
# 本リポ（PrimeBrains/moira の checkout）の git 根から実行
